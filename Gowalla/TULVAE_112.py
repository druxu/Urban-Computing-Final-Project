...
# from compiler.ast import flatten -- python 2x
...
bata = 0.5
keep_prob = tf.Variable(0.5, dtype=tf.float32)
alpha = tf.Variable(0.1, dtype=tf.float32)
it_learning_rate = tf.Variable(0.001, dtype=tf.float32)
...
# tensor definition
# Using tf.Tensor for dynamic values during runtime
# keep_prob = tf.placeholder("float")
# alpha = tf.placeholder("float")
# it_learning_rate = tf.placeholder("float")
input_x = tf.Variable(
    initial_value=tf.zeros([batch_size], dtype=tf.int32), dtype=tf.int32
)

l_y = tf.Variable(initial_value=tf.zeros([batch_size, label_size]), dtype=tf.float32)
vae_y = tf.Variable(
    initial_value=tf.zeros([batch_size, None, label_size]), dtype=tf.float32
)
vae_y_u = tf.Variable(
    initial_value=tf.zeros([label_size, batch_size, None, label_size]), dtype=tf.float32
)
...
un_target_sequence_length = tf.placeholder(
    tf.int32, [None], name="target_sequence_length"
)
un_max_target_sequence_length = tf.reduce_max(
    un_target_sequence_length, name="max_target_len"
)
...
def extract_character_vocab(total_T):
    ...
    set_words = list(set(chain(total_T)))
...
def readtraindata():
    ...
    test_UserT = list()
    test_lens = list()
    ftraindata = open("data/gowalla_scopus_1104.dat", "r")
    tempT = list()
    pointT = list()
    userT = list()
    seqlens = list()
    ...
    for index in range(len(pointT)):
        if userT[index] != flag or index == (len(pointT) - 1):
            ...
            if count > 1:  #
                ...
                temp_pointT += pointT[
                    int((index - count)) : int((index - count * rate))
                ]
                temp_userY += userT[int((index - count)) : int((index - count * rate))]
                temp_seqlens += seqlens[
                    int((index - count)) : int((index - count * rate))
                ]
            else:
...
def encoder(encoder_embed_input, y, keep_prob=0.5, reuse=False):
    with tf.variable_scope("encoder", reuse=reuse):
        encoder_input = tf.nn.embedding_lookup(dic_embeddings, encoder_embed_input)
        input_ = tf.transpose(encoder_input, [1, 0, 2])
        encode_lstm = tf.contrib.rnn.LSTMCell(
            n_hidden, forget_bias=1.0, state_is_tuple=True
        )
        encode_cell = tf.contrib.rnn.DropoutWrapper(
            encode_lstm, output_keep_prob=keep_prob
        )
        (outputs, states) = tf.nn.dynamic_rnn(
            encode_cell, input_, time_major=True, dtype=tf.float32
        )
        new_states = tf.concat([states[-1], y], 1)
        o_mean = tf.contrib.layers.fully_connected(
            inputs=new_states, num_outputs=z_size, activation_fn=None, scope="z_mean"
        )
        o_stddev = tf.contrib.layers.fully_connected(
            inputs=new_states, num_outputs=z_size, activation_fn=None, scope="z_std"
        )
...
def decoder(
    ...
    with tf.variable_scope("decoder", reuse=reuse):
        decode_lstm = tf.contrib.rnn.LSTMCell(
            n_hidden, forget_bias=1.0, state_is_tuple=True
        )
        decode_cell = tf.contrib.rnn.DropoutWrapper(
            decode_lstm, output_keep_prob=keep_prob
        )
        decoder_initial_state = encode_state
        output_layer = Dense(TOTAL_SIZE)  # TOTAL_SIZE
        decoder_input_ = tf.concat(
            [tf.fill([batch_size, 1], vocab_to_int["<GO>"]), decoder_embed_input], 1
        )  # add GO to the end
        ...
        masks = tf.sequence_mask(
            target_length, max_target_length, dtype=tf.float32, name="masks"
        )
...
def get_cost_c(pred):  # compute classifier cost
    cost = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=pred, labels=l_y)
    )
    return cost
...
def get_cost_l(
    ...
    latent_loss = 0.5 * tf.reduce_sum(
        tf.exp(z_stddev) - 1.0 - z_stddev + tf.square(z_mean), 1
    )
    latent_cost = tf.reduce_mean(latent_loss)

    encropy_loss = tf.contrib.seq2seq.sequence_loss(
        training_logits, target, masks
    )  # /batch_size
...
def get_cost_u(u_encoder_embed_input, u_decoder_embed_input):
    ...
    for label in range(label_size):
        ...
        cost_l, en_cost, kl_cost, training_logits = get_cost_l(
            u_encoder_embed_input,
            u_decoder_embed_input,
            [y_i] * batch_size,
            vae_y_u[label],
    ...
    U = (1.0 / label_size) * tf.reduce_sum(
        tf.multiply(L_ulab, prob_y) - tf.multiply(prob_y, tf.log(prob_y))
    )  #
...
cost_l, encropy_loss, latent_cost, training_logits = get_cost_l(
    l_encoder_embed_input,
    l_decoder_embed_input,
    l_y,
    vae_y,
    target_sequence_length,
    max_target_sequence_length,
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(iter_num):
            ...
            while step < len(new_trainT) // batch_size:
                ...
                un_input_batch = pad_sentence_batch(
                    un_encode_batch, vocab_to_int["<PAD>"]
                )
                ...
                for y_i in range(start_i, start_i + batch_size):
                    ...
                    TRAIN_DIC.get(get_mask_index(new_trainU[y_i], User_List))[2] += (
                        1  # Groud value Groud Truth a+c
                    )
                ...
                pred_batch, c_pred, op, batch_cost, l_cost, u_cost, c_cost = sess.run(
                    ...
                    feed_dict={
                        ...
                        u_decoder_embed_input: un_input_batch,
                        it_learning_rate: initial_learning_rate,
                        latentscale_iter: min_kl_epoch,
                        keep_prob: 0.5,
                        l_decoder_embed_input: input_batch,
                        target_sequence_length: pad_source_lengths,
                        un_target_sequence_length: un_pad_source_lengths,
                        alpha: alpha_epoch,
                    },
                )
                # computing
            ...
            print(
                ...
                acc / (step * batch_size),
                "Accuracy5",
                acc5 / (step * batch_size),
                "TRAIN LOSS",
                train_cost,
            )
            print("\nepoch TEST")
            TEST_p, TEST_r, TEST_f1, TEST_acc1, TEST_acc5 = test_model(
                sess, testT, testU, epoch
            )
        ...
        save_metrics(
            ...
            TEST_ACC1,
            TEST_ACC5,
            root="./out/gw_tulvae_test_112.txt",
        )
        save_metrics(
            Learning_rate,
            TRAIN_P,
        ...
        draw_pic_metric(
            TRAIN_P, TRAIN_R, TRAIN_F1, TRAIN_ACC1, TRAIN_ACC5, name="train"
        )
...
def test_model(sess, testT, testU, epoch):
    ...
    while step < len(testT) // batch_size:  #
        ...
        for y_i in range(start_i, start_i + batch_size):
            ...
            TEST_DIC.get(get_mask_index(testU[y_i], User_List))[2] += (
                1  # Groud value Groud Truth a+c
            )
            batch_y.append(xsy_step)
        c_pred, pred_batch = sess.run(
            [correct_pred, pred],
            feed_dict={
                l_encoder_embed_input: sources_batch,
                l_y: batch_y,
...
def draw_pic_metric(TEST_P, Test_R, Test_F1, Test_ACC1, TEST_ACC5, name="train"):
    font = {"family": name, "weight": "bold", "size": 18}
    width = 12
    height = 12
